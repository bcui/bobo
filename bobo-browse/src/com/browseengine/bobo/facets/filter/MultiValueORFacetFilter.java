...
public class MultiValueORFacetFilter extends RandomAccessFilter
{
  ...
  private final static class MultiValueOrFacetDocIdSetIterator extends FacetOrDocIdSetIterator
  {
      ...
      {
        return (_doc = (_doc < _maxID ? _nestedArray.findValues(_bitset, (_doc + 1), _maxID, false) : NO_MORE_DOCS));
      }
      ...
      final public int advance(int id) throws IOException
      {
        if (_doc < id)
        {
          return (_doc = (id <= _maxID ? _nestedArray.findValues(_bitset, id, _maxID, false) : NO_MORE_DOCS));
        }
        return nextDoc();
      }
  }
